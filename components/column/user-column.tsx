...
function getUserStatusDotClass(
  status: AdminUserStatus,
) {
  if (status === "ACTIVE") {
...
function getDocumentStatusClass(
  status: AdminUserDocumentStatus,
) {
  if (status === "VERIFIED") {
    return "border-transparent bg-emerald-50 text-emerald-700";
  }
...

    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Badge
            variant="outline"
            className={`gap-1.5 px-2.5 py-1 font-medium ${getUserStatusClass(
              status,
            )}`}
          >
            <span
              className={`size-1.5 rounded-full ${getUserStatusDotClass(
                status,
              )}`}
            />
...

    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Badge
            variant="outline"
            className={`px-2.5 py-1 font-medium ${getDocumentStatusClass(
              status,
            )}`}
          >
